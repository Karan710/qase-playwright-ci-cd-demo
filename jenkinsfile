pipeline {
    agent {
        docker {
            image 'node:18-alpine'
        }
    }

    parameters {
        string(name: 'QASE_PROJECT_CODE')
        string(name: 'QASE_RUN_ID')
        string(name: 'QASE_REPORT')
        string(name: 'QASE_API_BASE_URL')
        string(name: 'QASE_RUN_COMPLETE')
    }

    environment {
        QASE_TESTOPS_API_TOKEN = credentials('QASE_API_TOKEN')
    }

    stages {
        stage('Setup Environment Variables') {
            steps {
                script {
                    env.QASE_TESTOPS_PROJECT = params.QASE_PROJECT_CODE
                    env.QASE_TESTOPS_RUN_ID = params.QASE_RUN_ID

                    // ✅ handle "1", "true", true
                    def isComplete = params.QASE_RUN_COMPLETE in ['1', 'true', true]

                    env.QASE_TESTOPS_RUN_COMPLETE = isComplete ? 'true' : 'false'
                }
            }
        }

        stage('Run tests') {
            steps {
                git url: 'https://github.com/Karan710/qase-playwright-ci-cd-demo.git', branch: 'Karan'
                sh 'npm install'
                sh 'npx playwright test'
            }
        }
    }
}
